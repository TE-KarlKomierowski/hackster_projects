...
void Adafruit_SPITFT::writePixel(int16_t x, int16_t y, uint16_t color) {
  if ((x >= 0) && (x < _width) && (y >= 0) && (y < _height)) {
    if (this->frame_buffer) {
            this->frame_buffer[x + y * _width] = color;
    } else {
...
// // used solo -- as graphics primitives in themselves, not invoked by higher-d
...
//void Adafruit_SPITFT::drawRGBBitmap(void *buf, int16_t w, int16_t h) { /* smart meter*/
void Adafruit_SPITFT::drawRGBBitmap(void) { /* smart meter*/
    uint32_t count;

    //hwspi._spi->setBitOrder(LSBFIRST);
    uint16_t *buf16 = this->frame_buffer;
    count = _width * _height * 2;
    ...
    startWrite();
    setAddrWindow(0, 0,_width, _height); // Clipped area

    hwspi._spi->transfer(this->frame_buffer, count);
    endWrite();
