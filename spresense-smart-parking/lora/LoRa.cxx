...
/* SPI related*/
void SPIClass::beginTransaction(int spi) { SPI_SELECT(spidev, 0, true); };

uint16_t SPIClass::transfer(uint16_t data) {
  //  	SPI_SEND(spidev, data);
  uint16_t in_data;

  SPI_EXCHANGE(spidev, &data, &in_data, 1);

  return in_data;
};
void SPIClass::endTransaction() { SPI_SELECT(spidev, 0, false); };

void SPIClass::begin() {
  spidev = spi_init();
  // SPI_SETFREQUENCY(spidev, LORA_DEFAULT_SPI_FREQUENCY);
  SPI_SETFREQUENCY(spidev, 1000);
}
...
float LoRaClass::packetSnr() { return ((int8_t)readRegister(REG_PKT_SNR_VALUE)) * 0.25; }

long LoRaClass::packetFrequencyError() {
  int32_t freqError = 0;
  freqError = static_cast<int32_t>(readRegister(REG_FREQ_ERROR_MSB) & 0x7);
  freqError <<= 8L;
...
void LoRaClass::idle() { writeRegister(REG_OP_MODE, MODE_LONG_RANGE_MODE | MODE_STDBY); }
...
int LoRaClass::getSpreadingFactor() { return readRegister(REG_MODEM_CONFIG_2) >> 4; }
...
void LoRaClass::enableCrc() { writeRegister(REG_MODEM_CONFIG_2, readRegister(REG_MODEM_CONFIG_2) | 0x04); }

void LoRaClass::disableCrc() { writeRegister(REG_MODEM_CONFIG_2, readRegister(REG_MODEM_CONFIG_2) & 0xfb); }
